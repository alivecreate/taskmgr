          ...
          <div class="modal-content">
            ...
            <form id="eventForm" method="post">
                <div class="modal-body">
                    <textarea required
                    id="title" name="title" class="title form-control" placeholder="Event Title Here..." rows="" cols=""></textarea>
                    <p class="resDate"></p>
                    <br>
                    <input required type="datetime-local" 
                    id="start" name="start" class="start form-control" rows="" cols=""> 
                    
                    <br>
                    <input required type="datetime-local" 
                    id="end" name="end" class="end form-control" rows="" cols="">
            ...
            <datalist id="reds">
              ...
              <option>#000000</option>
                ...
                <button type="submit" id="saveEvent" class="btn btn-primary saveEvent" >Save changes</button>
                </div>
            </form>
          </div>
        </div>
...
$(".calendar").addClass( "menu-is-opening menu-open");
$(".calendar a").addClass( "active-menu");
...
//   alert('start -d ' + day);
  ...
  var startDate = [year, month, day].join('-')+'T'+[hour, minutes].join(':');
  return $.fullCalendar.formatDate(str, 'Y-MM-DD')+'T'+$.fullCalendar.formatDate(str, 'HH:mm');
  return startDate ;
};
...
//   if(hour == 5) hour = 
  ...
  if(minutes.length < 2) minutes = '0' + minutes;
  if(seconds.length < 2) seconds = '0' + seconds;
  var endDate = [year, month, day].join('-')+'T'+[hour, minutes].join(':');
...
//   timeFormat: 'H(:mm)', // uppercase H for 24-hour clock
        ...
        var StartHour = parseInt($.fullCalendar.formatDate(start, 'HH'));
        document.getElementById("start").defaultValue = convertStart(start);
        document.getElementById("end").defaultValue = convertEnd(end);
        //   document.getElementById("title").defaultValue = convertStart(start);
        // alert(($('#title').val().length));

        // console.log('click length- ' + $('#eventForm').html());

        // console.log($('#end').length);
        
        //  $('#eventForm').one('submit',function (e) { 

        //  $('#saveEvent').one('click',function (e) { 
         $('#saveEvent').one('click',function (e) { 
            
          e.preventDefault();
         console.log('submit length- ' + this.length);
         ...
         if(title)
            {
                $.ajax({
                    url:"/admin/full-calender/action2",
                    type:"POST",
                    data:{
                        title: title,
                        start: start,

...
<div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h2 class="text-danger">Report: <span class="text-dark">સ્ટેટ્સનું રિપોર્ટ</span></h2>
          </div>
          <div class="col-sm-6">
...
<option value="1" @if(isset($_REQUEST['status']) && ($_REQUEST['status'] == '1')) selected @endif>Pending</option>
<option value="2" @if(isset($_REQUEST['status']) && ($_REQUEST['status'] == '2')) selected @endif>Processing</option>
<option value="3" @if(isset($_REQUEST['status']) && ($_REQUEST['status'] == '3')) selected @endif>Canceled</option>
<option value="4" @if(isset($_REQUEST['status']) && ($_REQUEST['status'] == '4')) selected @endif>Completed</option>

</select>
                          <input type="submit" name="type" value="status-wise">
                        </form>
                  ...
                  <tbody>
                    @foreach($statusWiseData as $i => $statusWise)
                      <tr>
                        <td>{{++$i}}</td>
                        <td>{{dateFormat($statusWise->task_assign_date, 'd-M-Y')}}</td>
                        <td>
                        <span class="{{getStatusBadgeColor($statusWise->status_name)}}">
                        
                          <span class="text-captalize">{{$statusWise->status_name}}</span>
                          </span>
                        ...
                        <a href="{{route('task-assign.show',$statusWise->task_assign_id)}}?employee={{getEmployee($admin_group->id)->id}}">
                        ...
                        <div class="d-flex flex-row">
                                ...
                                @if(isset($admin_group->image))
                                  ...
                                  <p class="pull-left mr-2 text-strong text-danger">
                                      <img class="img-circle elevation-2 object-fit"  height="26" width="26"
                                  ...
                                  @else
                                        <p class="pull-left mr-2 text-strong text-danger">
                                    <img class="img-circle elevation-2"   height="26" width="26"
                      ...
                      <td>
                          ...
                          <td>{{$statusWise->task_description}}</td>
                         

                        <td class="">
                        

                        @if(getParents($statusWise->category_id)['category'])
                          <span class='badge badge-primary p-1'>{{getParents($statusWise->category_id)['category']->name}}</span>
                        @endif

                        
                        @if(getParents($statusWise->category_id)['subcategory'])
                          <span class='badge badge-danger p-1'>{{getParents($statusWise->category_id)['subcategory']->name}}</span>
                        @endif

                        
                        @if(getParents($statusWise->category_id)['subcategory2'])
                          <span class='badge badge-warning p-1'>{{getParents($statusWise->category_id)['subcategory2']->name}}</span>
                        @endif
                        ...
                        <a href="{{route('task-assign.show',$statusWise->task_assign_id)}}" class="btn btn-xs btn-warning float-left mr-2"  title="Task Details"><i class="fa fa-eye"></i></a>
                        
                      @if(session('LoggedUser')->id == 1)
                        <a href="{{route('task-assign.edit',$statusWise->task_assign_id)}}" class="btn btn-xs btn-info float-left mr-2"  title="Edit task"><i class="far fa-edit"></i></a>
                        <button class="btn btn-xs btn-danger del-modal float-left"  title="Delete task"  data-id="{{$statusWise->task_assign_id}}"
                         data-title="{{$statusWise->task_description}}"  data-toggle="modal" data-target="#modal-default"><i class="fas fa-trash-alt"></i>
                          </button>
                      @endif
